...
public class DataModel {
     ...
     */
    public List<model.BookAuthor> getListOfBooksByAuthor() {
        String query = "SELECT * FROM " + TABLE_BOOKS_AUTHOR_VIEW;

        try (Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery(query)) {

            List<model.BookAuthor> booksByAuthor = new ArrayList<>();

            while (results.next()) {
                model.BookAuthor bookAuthor = new model.BookAuthor();
     ...
     */
    public List<model.BookAuthor> getListOfAvailableBooks() {

        try (Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery(SELECT_AVAILABLE_BOOKS)) {

            List<model.BookAuthor> availableBooks = new ArrayList<>();
     ...
     */
    public List<model.BookAuthor> getListOfLoanedBooks() {

        try (Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery(SELECT_LOANED_BOOKS)) {

            List<model.BookAuthor> loanedBooks = new ArrayList<>();
     ...
     */
    public List<model.BookAuthor> getListOfBooksLoanedByMember(String firstName, String lastName) {
     ...
     */
    public List<model.LoanedBook> getLoanedBooks(String memberFirstName, String memberLastName) {
        List<model.LoanedBook> loanedBooks = new ArrayList<>();
        ...
        try {
            ...
            while (results.next()) {
                model.LoanedBook loanedBook = new model.LoanedBook();
    ...
    public boolean isBookLoanedByMember(String title, String memberFirstName, String memberLastName) {
        List<model.LoanedBook> loanedBooksByMember = getLoanedBooks(memberFirstName, memberLastName);
        int bookId = selectBookIdByTitle(title);

        for (model.LoanedBook loanedBook : loanedBooksByMember) {
     ...
     */
    public List<model.BookAuthor> searchBooksByTitle(String title) {
        List<model.BookAuthor> bookAuthors = new ArrayList<>();
     ...
     */
    public List<model.BookAuthor> searchBooksByAuthor(String[] authorName) {
        List<model.BookAuthor> bookAuthors = new ArrayList<>();
        String firstName;
        String lastName;
     ...
     */
    public List<model.BookAuthor> searchBooksBySubject(String subject) {
        List<model.BookAuthor> bookAuthors = new ArrayList<>();
    ...
    /**
     * Depending on the status of the DEVELOPMENT flag,
     * the connection will be made to 1 of the 2 databases
     * present in the project. One of them will be in
     * src\main\resources and the other will be in the .jar
     * file of the application. If DEVELOPMENT == true, then
     * use the database from src\main\resources. Otherwise,
     * use the database from the same directory that contains
     * the application .jar file.
     *
     * @return the full string needed to connect to library.db
     */
    private static String getConnectionString() {
        String connectionString;

        // Get the path of LibraryManagemenySystem.class
        File classInJarFile = new File(LibraryManagementSystem.class.getProtectionDomain()
                .getCodeSource().getLocation().getPath());

        // If it's in the .jar, then the path will be like this:
        // \Library Management System.jar\BOOT-INF\classes\application\LibraryManagementSystem.class
        // If it's in the target folder, then the path will be like:
        // \target\classes\application\LibraryManagementSystem.class
        File jarFile = DEVELOPMENT ? classInJarFile.getParentFile().getParentFile() :
                classInJarFile.getParentFile().getParentFile().getParentFile();
        String jarFilePath = jarFile.getPath();

        // Replace '%20' with spaces (' ')
        // and remove 'file:\' from the final path
        jarFilePath = jarFilePath.replaceAll("%20", " ");
        jarFilePath = jarFilePath.replace("file:\\", "");

        connectionString = "jdbc:sqlite:" +
                jarFilePath +
                (DEVELOPMENT ? "\\src\\main\\resources\\" : "\\") +
                DB_NAME;
